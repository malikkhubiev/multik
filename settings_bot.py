...
async def process_business_file_with_deepseek(file_content: str) -> str:
    ...
    try:
        ...
        payload = {
            "model": "deepseek-chat",
            "messages": [
                {"role": "system", "content": "Ты - эксперт по анализу информации. Твоя задача - извлечь из данных ключевую информацию, убрать лишние детали и символы и представить её в самом компактном виде без потери смысла"},
