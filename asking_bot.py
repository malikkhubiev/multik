...
async def get_or_create_dispatcher(token: str, business_info: str):
    ...
    async def handle_question(message: types.Message):
        ...
        if projects and len(projects) > 0:
            ...
            answer_context = f"Контекст для ответа: {projects[0]['business_info']}"
        ...
        state_data = await state.get_data()
        history = state_data.get("history", [])
        history_part_of_prompt = ""
        if len(history) > 0:
            history_part_of_prompt=f"История общения:{[h for h in history]}" 
        # Структура промпта
        prompt = role_base + answer_context + history_part_of_prompt + (form_purpose or 'чтобы мы могли связаться и сделать индивидуальное предложение')
